// ============================================================
        ...
        //  STATE
        // ============================================================
        let cart = JSON.parse(localStorage.getItem('nanzCart')) || [];
        let wishlist = JSON.parse(localStorage.getItem('nanzWishlist')) || [];
        let orders = JSON.parse(localStorage.getItem('nanzOrders')) || [];
        let currentUser = JSON.parse(localStorage.getItem('nanzUser')) || null;
        ...
        let bc;
        try {
            bc = new BroadcastChannel('nanz_shop_channel');
            bc.onmessage = (event) => {
                if (event.data.type === 'orders_updated' || event.data.type === 'chat_updated') {
                    orders = JSON.parse(localStorage.getItem('nanzOrders')) || [];
                    renderOrdersList();
        ...
        function updateUnreadBadges() {
            ...
            orders.forEach(order => {
                ...
                // Jika pesan terakhir dari admin, berarti user (pelanggan) belum baca/balas
                if (lastMsg.from === 'admin') {
                    if (currentUser && order.userId === currentUser.id) {
                        totalUnreadUser++;
                    }
                } 
                // Jika pesan terakhir dari user, berarti admin belum balas
                else if (lastMsg.from === 'user') {
                    totalUnreadAdmin++;
                }
            });

            // Update badge di bottom nav untuk user
            ...
            // Update badge di dashboard admin
        ...
        function handleGoogleLogin(response) {
            ...
            localStorage.setItem('nanzUser', JSON.stringify(currentUser));
            updateUserUI();
        ...
        document.getElementById('profileLogoutBtn').addEventListener('click', () => {
            if (confirm('Apakah Anda yakin ingin keluar?')) {
                currentUser = null;
                localStorage.removeItem('nanzUser');
                updateUserUI();
                document.getElementById('profileOverlay').classList.remove('open');
        ...
        const savedTheme = localStorage.getItem('nanzTheme') || (window.matchMedia('(prefers-color-scheme: light)').matches ? 'light' : 'dark');
        ...
        themeToggle.addEventListener('click', () => {
            ...
            localStorage.setItem('nanzTheme', next);
        ...
        function updateCartUI() {
            ...
            localStorage.setItem('nanzCart', JSON.stringify(cart));
        }
        ...
        document.getElementById('checkoutForm').addEventListener('submit', (e) => {
            ...
            const orderId = 'NANZ-' + Math.random().toString(36).substr(2, 4).toUpperCase() + '-' + Math.random().toString(36).substr(2, 4).toUpperCase();
            ...
            const order = {
                ...
                chat: [
                    { 
                        from: 'admin', 
                        text: `Halo ${currentUser.name}! Terima kasih telah memesan. Tim kami akan segera memproses pesanan #${orderId}.<br><br>Silakan selesaikan pembayaran Anda melalui tombol di bawah ini:`, 
                        paymentBtn: true,
            ...
            localStorage.setItem('nanzOrders', JSON.stringify(orders));
            broadcastOrders();
            ...
            // Langsung arahkan ke menu pesanan dan buka chat
            showToast('Pesanan Dibuat', `ID: ${orderId}. Selesaikan pembayaran di menu Chat.`, 'success', 5000);
            navigateTo('orders');
            setTimeout(() => {
        ...
        function renderOrderChatMessages() {
            ...
            container.innerHTML = order.chat.map(c => {
                ...
                const payBtnHtml = c.paymentBtn ? `<a href="https://nanz-payment.vercel.app" target="_blank" class="chat-payment-btn"><i class="fas fa-credit-card"></i> PAYMENT</a>` : '';
        ...
        //  ADMIN PANEL SYSTEM
        // ============================================================
        // Secret access: click logo 5 times
        ...
        function enterAdminMode() {
            localStorage.setItem('nanzLastPage', 'admin');
        ...
        function openAdminChat(orderId) {
            ...
            // Tandai pesan user sebagai dibaca oleh admin
            if (order.chat.length > 0 && order.chat[order.chat.length - 1].from === 'user') {
                broadcastChat(orderId); // Memicu update badge di perangkat lain
            }
        ...
        // Export / Import (Disabled since using Cloud Database)
        /*
        if (document.getElementById('adminExportBtn')) {
        ...
        */
        ...
        (function() {
            document.getElementById('sswebBtn').addEventListener('click', async () => {
                ...
                try {
                    const apiUrl = `https://api.screenshotmachine.com/?key=${CONFIG.ssMachineKey}&url=${encodeURIComponent(finalUrl.href)}&dimension=1024x768&cacheLimit=0`;
        ...
        function navigateTo(page) {
            ...
            localStorage.setItem('nanzLastPage', page);
        ...
        function renderProfilePage() {
            ...
            if (currentUser) {
                ...
                document.getElementById('btnProfileLogoutPage').onclick = () => {
                    if (confirm('Apakah Anda yakin ingin keluar?')) {
                        currentUser = null; localStorage.removeItem('nanzUser');
        ...
        // View-all buttons removed along with search
        ...
        // Real-time time updater (every minute)
        setInterval(() => {
        ...
        renderMenus();
        updateCartUI();
        showToast('Selamat Datang', 'NANZ SHOP siap melayani!', 'success', 4000);

        // ============================================================
        //  PAGE PERSISTENCE - Restore last active page on refresh
        // ============================================================
        (function restoreLastPage() {
            const lastPage = localStorage.getItem('nanzLastPage');
            if (!lastPage) return;
            ...
            // Only restore if we're on the same page file
            const currentFile = currentPath.split('/').pop() || 'index.html';
            const expectedPage = pageMap['/' + currentFile] || 'home';

            // If lastPage doesn't match current file, redirect to correct file
            if (lastPage !== expectedPage && lastPage !== 'admin') {
                const fileMap = {
                    'home': 'index.html',
                    'orders': 'pesanan.html',
                    'profile': 'profil.html'
                };
                const targetFile = fileMap[lastPage];
                if (targetFile && targetFile !== currentFile) {
                    // Don't auto-redirect to avoid loops, just update nav state
                }
            }

            // Update bottom nav to reflect current page
            document.querySelectorAll('.bottom-nav .nav-item').forEach(item => {
